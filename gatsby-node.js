...
exports.createPages = async gatsbyUtilities => {
  ...
  // Query our Success stories from the GraphQL server
 ...
 Promise.all(
  successStories.map(({ post }) =>
     ...
     gatsbyUtilities.actions.createPage({
       // Use the WordPress uri as the Gatsby page path
       // This is a good idea so that internal links and menus work 👍
       path: `/success-stories/${post.slug}`,
...
/**
 * This function queries Gatsby's GraphQL server and asks for
 * All WordPress Success Story custom posts. If there are any GraphQL error it throws an error
 * Otherwise it will return the posts 🙌
 *
 * We're passing in the utilities we got from createPages.
 * So see https://www.gatsbyjs.com/docs/node-apis/#createPages for more info!
 */
 async function getSuccessStories({ graphql, reporter }) {
  const graphqlResult = await graphql(/* GraphQL */ `
    query WpSuccessStories {
      # Query all WordPress Success Story custom posts sorted by date
  ...
  if (graphqlResult.errors) {
    reporter.panicOnBuild(
      `There was an error loading your success story posts`,
      graphqlResult.errors
    )
    return
  }
